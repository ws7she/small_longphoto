...
var ctx = wx.createCanvasContext('comCanvas');
var windowWidth;
Page({
    data: {
    ...
    onLoad: function (options) {
        ...
        for (var i in listAll) {
            ...
            if (listAll[i].type == "picture") {
                ctx.scale(listAll[i].scaleX,  listAll[i].scaleY)
                ctx.rotate(listAll[i].skew * Math.PI / 180)
                ctx.save();
                ctx.drawImage(listAll[i].url, listAll[i].left, listAll[i].top, 200, listAll[i].height);
                ctx.restore();
